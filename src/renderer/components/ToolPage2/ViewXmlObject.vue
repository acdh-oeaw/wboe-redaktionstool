		...
		<b-card :header="content.n" no-body class="mib10 paneldecent">
			...
			<b-collapse v-model="isOpen" :id="'collapse-' + _uid">
				<b-card-body>
					...
					<div v-if="content.v">
						<button @click="valueOpen = !valueOpen" class="btn-none"><b>Value{{ ((processOpen) ? ':' : '') }}</b> <font-awesome-icon :icon="((valueOpen) ? 'eye' : 'eye-slash')" class="fa-icon mil5"/></button><br>
						<code class="lb val" v-if="valueOpen">{{ content.v }}</code>
					</div>
					<div v-if="content.xml">
						<button @click="xmlOpen = !xmlOpen" class="btn-none"><b>HTML{{ ((processOpen) ? ':' : '') }}</b> <font-awesome-icon :icon="((xmlOpen) ? 'eye' : 'eye-slash')" class="fa-icon mil5"/></button><br>
